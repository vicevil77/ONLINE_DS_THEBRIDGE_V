...
def features_anova(X_train, y_train, k=5):
    # Calcular las puntuaciones de la prueba ANOVA para cada característica
    f_scores, _ = f_classif(X_train, y_train)
    
    # Obtener los índices de las características ordenadas por las puntuaciones
    feature_indices = sorted(range(len(f_scores)), key=lambda i: f_scores[i], reverse=True)
    
    # Seleccionar las k características con las puntuaciones más altas
    selected_features_anova = [X_train.columns[i] for i in feature_indices[:k]]
