...
if 'coordenadas' not in df.columns:
    df['coordenadas'] = df["IP Origen"].apply(geocode_ips)
...
for idx, row in df.iterrows():
    if contador_ips_con_coordenadas + contador_ips_sin_coordenadas == 10000:
        print("Se ha alcanzado el limite de 100000 IPs. Guardando resultados en HTML...")
        # Guardar el DF como un archivo HTML
        df.to_html(r"E:\Cursos\BC_Data_Science\Repositorio\ONLINE_DS_THEBRIDGE_V\proyecto EDA\coordenadas_ips_Origen.text")
        break

    coordenadas = geocode_ips(row["IP Origen"])
    if coordenadas:
...
print("Geolocalizacion completada. Resultados guardados en mapa_coordenadas.html.")
